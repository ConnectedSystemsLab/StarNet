...
os.makedirs("eval_wo/data", exist_ok=True)
os.makedirs("eval_wo/pics", exist_ok=True)
...
if '__main__' == __name__:
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = SatelliteGRU_attention(args.input_len, args.hidden_size, args.num_layers, args.output_len, feature_size,
                                   device,
    ...
    np.save(f'./eval_wo/data/eval_data.npy', data)
    ...
    for i in range(num_chunks):
        ...
        plt.savefig(f'./eval_wo/pics/predictions_chunk_{i + 1}.png')
        # plt.show()
        plt.close()